...
// 对明确密级和敏感身份/金融标识作保守阻断。输入为拟发送模型的有限材料，输出是否不得外发模型；
// 不使用“请假、出差、考勤、报销”等普通办公词，也不以孤立的“机密/秘密”子串误伤常规行政公文。
function containsRestrictedWritingContent(value: string) {
  return /(?:\bD4(?:级|密级|资料|文件|信息|数据)?\b|(?:绝密|机密|秘密)(?:级|资料|文件|信息|数据|事项|项目|附件)|身份证(?:号码|号)?|银行账号|个人隐私|核心敏感)/i.test(value);
}
