...
async function main() {
  ...
  await mustContain("../app/api/knowledge-import-batches/route.ts", ["canManageUploadRules", "D4/机密资料不能通过当前在线批量导入入口上传", "totalCount"]);
  ...
  await mustContain("../app/page.tsx", ["/api/knowledge-import-batches", "LOCAL_TRIAL_20260828", "batchFileStatuses", "multiple accept=\".docx,.pdf,.txt,.md,.xlsx,.xls,.pptx,.ppt\""]);
