...
const ocrConfig = validateModelConnectorInput({ purpose: "OCR", baseUrl: "http://127.0.0.1:8765", endpointPath: "/ocr", enabled: true });
assert.equal(ocrConfig.model, "PaddleOCR"); assert.equal(ocrConfig.endpointPath, "/ocr");
...
const ocr = await testModelConnector({ purpose: "OCR", baseUrl: "http://127.0.0.1:8765", model: "PaddleOCR", timeoutMs: 500, endpointPath: "/ocr", enabled: true }, "", async (url, options) => { assert.equal(String(url), "http://127.0.0.1:8765/health"); assert.equal(options?.method, "GET"); return Response.json({ ready: true }); });
assert.equal(chat.status, "CONNECTED"); assert.equal(embedding.status, "CONNECTED"); assert.equal(reranker.status, "CONNECTED"); assert.equal(ocr.status, "CONNECTED");
