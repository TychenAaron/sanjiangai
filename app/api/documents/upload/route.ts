...
/**
 * 上传一个正式知识文件。
 * 输入为管理员文件和可选批次 ID；输出为 pending_review 文档。格式、禁止词条检查均在 R2/D1 写入前完成。
 */
export async function POST(request: Request) {
  ...
  try {
    ...
    await db.insert(documents).values({
      ...
      parseStatus, indexStatus: parseStatus === "parsed" ? "ready" : "pending", resourceStatus: "pending_review", resourceCategory: batch?.resourceCategory || String(form.get("resourceCategory") || form.get("documentType") || "其他"),
      ...
      reliabilityScore: 0, knowledgeStatus: "pending", currentVersion: 1, createdBy: user.name, createdByUserId: user.id, datasetId: batch?.datasetId || null, importBatchId: batch?.id || null, createdAt: now, updatedAt: now,
    });
    await db.insert(documentVersions).values({ id: versionId, documentId, versionNo: 1, content, changeSummary: parseStatus === "parsed" ? "文件首次上传并自动解析" : `文件已保存，${parseReason || "等待后续解析"}`, versionStatus: "pending", createdBy: user.name, createdAt: now });
    ...
    await db.insert(auditLogs).values({ id: crypto.randomUUID(), action: "上传并解析文件", entityType: "document", entityId: documentId, operator: user.name, detail: `已进入待审核正式资料生命周期；解析状态：${parseStatus}`, createdAt: now });
