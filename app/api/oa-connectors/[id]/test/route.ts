...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const requestId = getRequestId(request); await db.update(oaConnectorConfigs).set({ lastCheckStatus: result.status, lastCheckHttpStatus: result.httpStatus, lastCheckDurationMs: result.durationMs, lastCheckedAt: checkedAt, updatedAt: checkedAt }).where(eq(oaConnectorConfigs.id, id)); await db.insert(auditLogs).values({ id: crypto.randomUUID(), action: "oa_connector_tested", entityType: "oa_connector_config", entityId: id, operator: user.id, detail: `OA 连接检测：${result.status}，耗时 ${result.durationMs}ms。｜request_id=${requestId}`, requestId, createdAt: checkedAt }); writeStructuredLog({ requestId, user, route: "oa.connectors.test", result: result.status, latencyMs: result.durationMs, errorCode: result.status === "CONNECTED" ? undefined : result.status }); return Response.json({ ...result, checkedAt });
