...
export async function POST(request: Request) {
  try {
    ...
    const requestId = getRequestId(request); await db.insert(auditLogs).values({ id: crypto.randomUUID(), action: "oa_connector_config_saved", entityType: "oa_connector_config", entityId: id, operator: user.id, detail: `OA 连接器配置已保存；未记录地址或凭证。｜request_id=${requestId}`, requestId, createdAt: now }); writeStructuredLog({ requestId, user, route: "oa.connectors.save", result: "success", latencyMs: 0 });
