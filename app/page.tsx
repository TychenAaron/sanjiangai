...
type DocumentRecord = {
  ...
  trialDataClass: string; isTrialData: boolean; fileName: string | null; parseStatus: string; indexStatus: string; vectorStatus: string;
...
type ModelConnectorRecord = { id: string; purpose: "MAIN_MODEL" | "EMBEDDING" | "RERANKER" | "OCR"; baseUrl: string; model: string; timeoutMs: number; endpointPath: string | null; enabled: boolean; hasCredentials: boolean; lastCheckStatus: string | null; lastCheckHttpStatus: number | null; lastCheckDurationMs: number | null; lastCheckedAt: string | null };
const MODEL_CONNECTOR_SPECS = [{ purpose: "MAIN_MODEL", label: "主模型（知识问答与智能写作）", path: false, model: true }, { purpose: "EMBEDDING", label: "Embedding", path: false, model: true }, { purpose: "RERANKER", label: "Reranker", path: true, model: true }, { purpose: "OCR", label: "OCR（项目内 PaddleOCR 服务）", path: true, model: false }] as const;
...
function Dashboard({ greeting, userName, isSystemAdmin, query, setQuery, ask, docType, setDocType, go, records, summary, dataLoading }: {
  greeting: string; userName: string; isSystemAdmin: boolean; query: string; setQuery: (v: string) => void; ask: (e: FormEvent) => void;
  ...
  return <>
    ...
    {isSystemAdmin && <section className="panel extensions">
...
// 把后端真实解析/向量状态转换为资料管理员可理解的文案，不推测 OCR 或转换是否已经完成。
function parseStatusLabel(record: DocumentRecord) {
  if (record.parseStatus === "failed") return "解析失败";
  if (record.parseStatus === "pending_ocr") return "待 OCR";
  if (record.parseStatus === "pending_conversion") return "待转换";
  if (record.parseStatus === "parsed" && record.vectorStatus === "pending") return "待向量化";
  if (record.parseStatus === "parsed") return "已解析";
  return "解析中";
}
...
function Library({ user, records, loading, error, refresh }: { user: SessionUser | null; records: DocumentRecord[]; loading: boolean; error: string; refresh: () => Promise<void> }) {
  ...
  async function saveUpload() {
    ...
    try {
      ...
      // 每份文件仍单独走既有服务端安全链：格式/大小、禁止词、R2 回滚、D1 版本与自动批准状态互不影响。
      for (const file of uploadFiles) {
        const extension = file.name.split(".").pop()?.toLowerCase() || "";
        if (!new Set(["doc", "docx", "pdf", "txt", "md", "xlsx", "xls", "pptx", "ppt"]).has(extension)) { const reason = "不支持的文件类型"; failed.push({ fileName: file.name, reason }); setBatchFileStatuses(items => items.map(item => item.fileName === file.name && item.size === file.size ? { ...item, status: "失败", reason } : item)); continue; }
  ...
  /**
   * 从受控原文件重新解析并重建单份或全部正式资料索引。
   * 输入为可选资料 ID；输出只展示计数，不读取或显示 R2 地址、正文或向量数据。
   */
  async function rebuildKnowledgeIndex(target?: DocumentRecord) {
    const targetLabel = target ? `“${target.title}”` : "全部可管理资料";
    if (!window.confirm(`确认重新解析并重建 ${targetLabel} 的知识索引？该操作不会修改资料权限、审批状态或原始文件。`)) return;
    setSaving(true); setNotice("");
    try {
      const response = await fetch("/api/knowledge-index/rebuild", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ documentIds: target ? [target.id] : undefined, reparse: true }) });
      const result = await response.json() as { total?: number; reparsed?: number; ready?: number; pending?: number; failed?: number; error?: string };
      if (!response.ok) throw new Error(result.error || "重新解析失败");
      setNotice(`重建完成：处理 ${result.total || 0} 份，重新解析 ${result.reparsed || 0} 份，向量就绪 ${result.ready || 0} 份，待处理 ${result.pending || 0} 份，失败 ${result.failed || 0} 份。`);
      await refresh();
    } catch (error) { setNotice(error instanceof Error ? error.message : "重新解析失败"); }
    finally { setSaving(false); }
  }
  ...
  return <section className="page"><PageTitle kicker="知识中枢" title="知识资源" text="统一管理OA转载、人工上传、政府政策、AI草稿、人工修改稿和最终定稿；原文、元数据、权限和版本同步留存。"/>
    ...
    <div className="library-actions"><div><strong>资料台账</strong><span>共 {records.length} 份真实记录</span></div>{canManageLibrary && <span className="form-actions"><button disabled={saving} onClick={() => void rebuildKnowledgeIndex()}>{saving ? "正在处理…" : "重建全部资料索引"}</button><button onClick={() => setAdding(!adding)}>{adding ? "取消导入" : "＋ 集团资料批量导入"}</button></span>}</div>
    ...
    {adding && <div className="panel ingest-form">
      ...
      <div className="form-grid">
        ...
        <label><span>来源单位</span><input value={(form as Record<string, string>).sourceOrganization || ""} onChange={e => setField("sourceOrganization", e.target.value)} placeholder="可选填写"/></label>
        ...
        <label><span>适用范围</span><input value={(form as Record<string, string>).applicableScope || ""} onChange={e => setField("applicableScope", e.target.value)} placeholder="可选填写"/></label>
      </div>
    ...
    <div className="panel library-table"><div className="table-head"><span>{canManageLibrary ? "选择 / 资料名称" : "资料名称"}</span><span>来源 / 类型</span><span>访问 / 可见性</span><span>状态</span><span>文件日期</span><span>操作</span></div>
      {loading ? <div className="table-empty">正在读取资料数据库…</div> : visibleRecords.length === 0 ? <div className="table-empty">当前筛选范围内没有可见资料。</div> : visibleRecords.map(d => <div className={`library-row${canManageLibrary ? " has-selection" : ""}`} key={d.id}>{canManageLibrary && <input aria-label={`选择 ${d.title}`} type="checkbox" checked={selectedDocumentIds.includes(d.id)} onChange={event => setSelectedDocumentIds(ids => event.target.checked ? [...new Set([...ids, d.id])] : ids.filter(id => id !== d.id))}/>}<button className={selected?.id === d.id ? "selected-row" : ""} onClick={() => { void openRecord(d); }}><span><i>{d.documentType[0]}</i><span><b>{d.title}</b><small>{d.ownerDepartment} · {d.permissionScope}{d.fileName ? ` · ${d.fileName.split(".").pop()?.toUpperCase()} · ${parseStatusLabel(d)}` : ""}</small></span></span><span>{d.resourceCategory || "未分类"}<small>{d.sourceOrganization || "待补充来源"}</small></span><span>系统默认</span><span><em className={`record-status ${d.resourceStatus}`}>{statusLabel(d.resourceStatus)}</em></span><span>{d.documentDate || "待补充"}</span></button>{d.fileName && <a className="library-preview-link" href={`/knowledge/resources/${encodeURIComponent(d.id)}/preview`} title={d.fileName}>预览文件</a>}</div>)}
    </div>
    {selected && <section className="panel version-panel"><div className="version-head"><div><strong>{selected.title}</strong><span>版本链 · 原始内容不覆盖 · {parseStatusLabel(selected)}</span></div>{canManageLibrary && <span className="form-actions"><button disabled={saving} onClick={() => void rebuildKnowledgeIndex(selected)}>重新解析</button><button onClick={() => void editMetadata(selected)}>编辑资料信息</button><button onClick={() => { setVersionEditor(!versionEditor); setVersionContent(versions[0]?.content || ""); }}>{versionEditor ? "取消修改" : "＋ 创建新版本"}</button></span>}</div>
...
function ModelConnectorAdmin({ isSystemAdmin }: { isSystemAdmin: boolean }) {
  ...
  const load = useCallback(async () => {
    ...
    const rows = data.connectors || []; setConnectors(rows); setForms(Object.fromEntries(MODEL_CONNECTOR_SPECS.map(spec => { const item = rows.find(row => row.purpose === spec.purpose); return [spec.purpose, { baseUrl: item?.baseUrl || "", model: item?.model || "", apiKey: "", timeoutMs: item?.timeoutMs || 15000, endpointPath: item?.endpointPath || (spec.purpose === "OCR" ? "/ocr" : "/rerank"), enabled: item?.enabled || false }]; })));
  }, [isSystemAdmin]);
  useEffect(() => { const timer = window.setTimeout(() => { void load(); }, 0); return () => window.clearTimeout(timer); }, [load]);
...
function OaConnectorAdmin({ isSystemAdmin }: { isSystemAdmin: boolean }) {
  ...
  return <section className="panel oa-connector-panel"><div className="control-head"><div><h2>OA 接入配置</h2><p>仅保存受控连接参数。凭证仅加密保存在服务端；“测试连接”只发起 GET/HEAD 探测，不同步、不导入资料。</p></div><b>管理员专用</b></div>
    <div className="oa-connector-section"><h3>连接参数</h3><div className="oa-connector-form"><label><span>OA 名称 *</span><input value={form.name} onChange={event => setForm({ ...form, name: event.target.value })}/></label><label><span>Base URL *</span><input value={form.baseUrl} onChange={event => setForm({ ...form, baseUrl: event.target.value })} placeholder="https://..."/></label><label><span>API Path / Endpoint *</span><input value={form.endpointPath} onChange={event => setForm({ ...form, endpointPath: event.target.value })} placeholder="/health"/></label><label><span>请求方法</span><select value={form.requestMethod} onChange={event => setForm({ ...form, requestMethod: event.target.value as "GET" | "HEAD" })}><option>GET</option><option>HEAD</option></select></label><label><span>Content-Type</span><input value={form.contentType} onChange={event => setForm({ ...form, contentType: event.target.value })}/></label><label><span>认证方式</span><select value={form.authType} onChange={event => setForm({ ...form, authType: event.target.value as OaConnectorRecord["authType"] })}><option value="NONE">NONE</option><option value="BEARER_TOKEN">BEARER_TOKEN</option><option value="API_KEY">API_KEY</option><option value="BASIC_AUTH">BASIC_AUTH</option><option value="CUSTOM_HEADER">CUSTOM_HEADER</option></select></label><label><span>自定义认证 Header 名</span><input value={form.customAuthHeaderName} onChange={event => setForm({ ...form, customAuthHeaderName: event.target.value })} placeholder="仅 CUSTOM_HEADER 使用"/></label><label><span>超时（毫秒）</span><input type="number" min={500} max={120000} value={form.timeoutMs} onChange={event => setForm({ ...form, timeoutMs: Number(event.target.value) || 15000 })}/></label><label><span>非敏感 Header JSON</span><input value={form.headersText} onChange={event => setForm({ ...form, headersText: event.target.value })} placeholder='{"Accept-Language":"zh-CN"}'/></label><label><span>启用测试连接</span><select value={form.enabled ? "yes" : "no"} onChange={event => setForm({ ...form, enabled: event.target.value === "yes" })}><option value="no">停用</option><option value="yes">启用</option></select></label></div></div>
    <div className="oa-connector-section"><h3>认证凭证</h3><div className="oa-connector-form oa-credential-form"><label><span>Token</span><input type="password" value={form.token} onChange={event => setForm({ ...form, token: event.target.value })}/></label><label><span>AppKey</span><input type="password" value={form.appKey} onChange={event => setForm({ ...form, appKey: event.target.value })}/></label><label><span>AppSecret</span><input type="password" value={form.appSecret} onChange={event => setForm({ ...form, appSecret: event.target.value })}/></label><label><span>API Key</span><input type="password" value={form.apiKey} onChange={event => setForm({ ...form, apiKey: event.target.value })}/></label><label><span>Username</span><input value={form.username} onChange={event => setForm({ ...form, username: event.target.value })}/></label><label><span>Password</span><input type="password" value={form.password} onChange={event => setForm({ ...form, password: event.target.value })}/></label><label><span>自定义 Header 值</span><input type="password" value={form.customHeaderValue} onChange={event => setForm({ ...form, customHeaderValue: event.target.value })}/></label><div className="oa-save-action"><button disabled={working === "save"} onClick={() => void save()}>{working === "save" ? "正在保存…" : editingId ? "保存修改" : "保存配置"}</button></div></div></div>
    {message && <div className="notice admin-notice">{message}</div>}
